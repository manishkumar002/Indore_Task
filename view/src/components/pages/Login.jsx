  ...
  const onSubmit = (data) => {
    ...
    .then((result) => {
      if (result.status === true && result.code === 200) {
        ...
        setServerError(result.errors ? result.errors[0]?.msg : result.message);
      }
    })
    .catch((err) => {
        ...
        <div className="row py-3 a1">
          ...
          <div className="col-sm-4 b1 my-4">
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {serverError && <p className="ErrorColor">{serverError}</p>}<br/>
                <div>
                  <button type="submit" className="form-control w-100" style={{ background: '#fdc700' }}>Login</button>
                </div>
              </form>
              <br />
